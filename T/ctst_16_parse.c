...
int tst(void)
{
  ...
  /* assume uint == 32bit */
  ASSERT(0xFFFFFFFF == ustr_parse_uint(USTR1(\x9, "100000000"), 16, &ern));
  ASSERT(!ern);
  ASSERT(0xFFFFFFFF == ustr_parse_uint(USTR1(\x9, "100000001"), 16, &ern));
  ASSERT(!ern);
  ASSERT(0 == ustr_parse_uint(USTR1(\x9, "100000001"), 16 | flg_ovr, &ern));
